...
function buildCounterpartyRows(aging: AgedBalance): CounterpartyRowModel[] {
  return aging.counterparties
    ...
    .filter((row) => row.status.totalAmount > 0)
    .sort(sortCounterpartyRows)
}
...
function CounterpartyDelay({ status }: { status: AgedBalanceCounterparty }) {
  ...
  return (
    <span
      ...
      style={{ color: AGING_BUCKET_COLOR[status.worstBucketKey] }}
...
function OverdueAmount({ status }: { status: AgedBalanceCounterparty }) {
  ...
  return (
    <span style={{ color: AGING_BUCKET_COLOR[status.worstBucketKey] }}>
