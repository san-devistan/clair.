...
export function MonthlyBarChart({
  ...
  const stackedCategories = stackedMetric
  ...
  const config = buildChartConfig(metric, stackedCategories)

  const data = monthly.map((m, i) => ({
    ...m,
    [COMPARISON_KEY]: comparison?.[i]?.[metric] ?? null,
    ...buildStackedDatum(m, stackedMetric, stackedCategories, comparison?.[i]),
  }))
  ...
  return (
    <ChartContainer config={config} className={className}>
      <BarChart data={data}>
        <ChartAxes />
        <ChartTooltip content={<MonthlyTooltipContent />} />
    ...
    <div className="flex w-full items-center justify-between gap-4">
      <span className="flex min-w-0 items-center gap-1.5 text-muted-foreground">
        <span
          ...
          style={{ background: tooltipItemColor(item) }}
        />
